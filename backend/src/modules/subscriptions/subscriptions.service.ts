...
export class SubscriptionsService {
  static getSubscriptionData(token: string) {
    ...
    // Build standard, clean proxy URIs according to Hiddify / Clash Meta specs
    for (const p of protocols) {
      if (p.protocol_type === 'hysteria2') {
        ...
        // Universal Hiddify TUIC URI format: password only or uuid as password with allow_insecure
        ...
        const vless = `vless://${user.uuid}@${host}:443?encryption=none&type=tcp&security=reality&pbk=${pubKeyStr}&fp=chrome&sni=dl.google.com&sid=${shortIdStr}&flow=xtls-rprx-vision#🛡️ VLESS REALITY (Camouflage) - ${user.username}`;
        uris.push(vless);
      }
    }
    ...
    if (uris.length === 0) {
      ...
      uris.push(`vless://${user.uuid}@${host}:443?encryption=none&type=tcp&security=reality&pbk=${pubKeyStr}&fp=chrome&sni=dl.google.com&sid=${shortIdStr}&flow=xtls-rprx-vision#🛡️ VLESS REALITY (Camouflage) - ${user.username}`);
    }

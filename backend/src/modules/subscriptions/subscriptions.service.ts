  ...
  static getSubscriptionData(token: string) {
    ...
    for (const p of protocols) {
      if (p.protocol_type === 'hysteria2') {
        ...
        // Strict TUIC v5 URI specification for Hiddify Next & sing-box core (no v4 udp_relay_mode)
        const tuic = `tuic://${user.uuid}:${user.uuid}@${host}:8444?congestion_control=bbr&allow_insecure=1&sni=${sniDomain}#⚡ TUIC v5 (Lage Latency) - ${user.username}`;
        uris.push(tuic);
